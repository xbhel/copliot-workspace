...
Provide a reusable template for creating structured unit testing skills across languages by composing the different guidance layers.
...
|max_iterations|Default iteration cap for the generated skill's test-and-fix loop (run tests → fix failures → improve coverage). Passed through to the Workflow section of the output skill.|3|No|user or default| |3|

## Context

This skill composes six layers into a complete unit testing skill. Each layer is a **checklist of required items** that must be filled with **concrete, target-specific details** — not left as generic prompts. The layers are defined by the template in [references/layers.md](references/layers.md).
...
2. **Compose the skill from the six layers:** Use [references/layers.md](references/layers.md) as the source template and populate each layer with concrete, target-specific guidance. Apply:  
    - Layers 1–3 to Context & Core Principles
    - Layer 4 to Workflow
    - Layer 5 to Output
    - Layer 6 to Error Handling
    - Layer 3 only when `{domain_context}` is provided
3. **Assemble and validate the final skill:** Compose a complete SKILL.md per AGENTS.md structure. Verify all `{variable}` references are defined in the Inputs table and ensure the generated skill does not depend on this skill's own reference files.
